"""
CtkSmartScrollableFrame: A drop-in replacement for CTkScrollableFrame that only shows the scrollbar when content overflows.

This is a complete copy of CTkScrollableFrame with smart scrollbar visibility added.
All scrolling functionality should work exactly like the original CTkScrollableFrame.

Usage:
    from ctk_smart_scrollable_frame import CtkSmartScrollableFrame
    frame = CtkSmartScrollableFrame(parent, ...)

Does not currently support changing orientation after init since self._orientation is only set then.
"""
...
class CtkSmartScrollableFrame(tkinter.Frame, CTkAppearanceModeBaseClass, CTkScalingBaseClass):
    def __init__(self,
                 master: Any,
                 width: int = 200,
                 height: int = 200,
        ...
        if self._orientation == "horizontal":
            ...
            self._parent_canvas.configure(xscrollcommand=self._smart_scrollbar_set)
        elif self._orientation == "vertical":
            self._scrollbar = CTkScrollbar(master=self._parent_frame, orientation="vertical", command=self._parent_canvas.yview,
                                           fg_color=scrollbar_fg_color, button_color=scrollbar_button_color, button_hover_color=scrollbar_button_hover_color)
            self._parent_canvas.configure(yscrollcommand=self._smart_scrollbar_set)
    ...
    def scroll_widget_into_view(self, widget, margin=5):
        """
        Scrolls the canvas so that the given widget is visible with the specified margin (in pixels).
        If the widget is both above and below (taller than the viewport), scroll so the top is visible.
        """
        ...
        if not scrollregion:
            return
        # Get widget's y position and height relative to the inner frame
        widget_y = widget.winfo_y()
        widget_height = widget.winfo_height()
        # Get current top and bottom of visible area
        y0 = int(canvas.canvasy(0))
        y1 = y0 + canvas_height
        # If widget is taller than the viewport or both above and below, scroll so top is visible
        if widget_height + 2 * margin > canvas_height or (widget_y < y0 and widget_y + widget_height > y1):
            new_top = max(widget_y - margin, 0)
            canvas.yview_moveto(new_top / (scrollregion[3] - scrollregion[1]))
        # If widget is above the visible area, scroll up
        elif widget_y - margin < y0:
            new_top = max(widget_y - margin, 0)
            canvas.yview_moveto(new_top / (scrollregion[3] - scrollregion[1]))
        # If widget is below the visible area, scroll down
        elif widget_y + widget_height + margin > y1:
            new_top = (widget_y + widget_height + margin) - canvas_height
            canvas.yview_moveto(new_top / (scrollregion[3] - scrollregion[1]))
        # Otherwise, it's already in view 